...
class Memory : public DigitalCircuit
{
  ...
  public:
    ...
    Memory(const std::string &name, const Wire<32> *iAddress, const Wire<32> *iWriteData, const Wire<1> *iMemRead,
           const Wire<1> *iMemWrite, Wire<32> *oReadData, const Endianness endianness,
    ...
    {
        std::uint32_t address = static_cast<std::uint32_t>(_iAddress->to_ulong());
        std::uint32_t writeData = static_cast<std::uint32_t>(_iWriteData->to_ulong());
        bool memRead = static_cast<bool>(_iMemRead->to_ulong());
        bool memWrite = static_cast<bool>(_iMemWrite->to_ulong());
        ...
        if (memRead)
        {
            ...
            if (_endianness == LittleEndian)
            {
                ...
                readData <<= 8;
                readData |= _memory[address + 2].to_ulong();
                readData <<= 8;
                readData |= _memory[address + 1].to_ulong();
                readData <<= 8;
        ...
        if (memWrite)
        {
            if (_endianness == LittleEndian)
            {
                ...
                writeData >>= 8;
                _memory[address + 1] = (writeData & 0xFF);
                writeData >>= 8;
                _memory[address + 2] = (writeData & 0xFF);
                writeData >>= 8;
