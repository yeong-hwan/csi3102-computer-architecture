...
class RegisterFile : public DigitalCircuit
{
  ...
  public:
    RegisterFile(const Wire<5> *iReadRegister1, const Wire<5> *iReadRegister2, const Wire<5> *iWriteRegister,
                 const Wire<32> *iWriteData, const Wire<1> *iRegWrite, Wire<32> *oReadData1, Wire<32> *oReadData2,
    ...
    virtual void advanceCycle()
    {
        ...
        uint32_t readReg1 = static_cast<uint32_t>(_iReadRegister1->to_ulong());
        uint32_t readReg2 = static_cast<uint32_t>(_iReadRegister2->to_ulong());
        uint32_t writeReg = static_cast<uint32_t>(_iWriteRegister->to_ulong());
        uint32_t writeData = static_cast<uint32_t>(_iWriteData->to_ulong());
        bool regWrite = static_cast<bool>(_iRegWrite->to_ulong());

        // Output the data from the read registers
        *_oReadData1 = _registers[readReg1];
        *_oReadData2 = _registers[readReg2];

        // Write data to the register if RegWrite is enabled and write register is not zero
